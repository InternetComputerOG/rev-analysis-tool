<script>
  ...
  let workspace = $derived(selectedKey ? productWorkspace(rows, selectedKey) : null)
</script>

{#if !rows.length}
  <EmptyState title="No product data" message="Adjust filters to compare revenue streams." />
{:else}
  <SectionHeader title="Revenue products" subtitle="Click a card to expand the product workspace" />
