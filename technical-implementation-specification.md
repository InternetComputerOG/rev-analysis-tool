...
| Hosting & delivery   | Cloudflare Workers static assets    | Free tier                            |
...
rev-analysis-tool/
├── public/
│   └── revenue-data.csv          # Local-only; gitignored (~170k rows, ~17 MB)
├── src/
│   ├── lib/
│   │   ├── data/
...
The file `public/revenue-data.csv` is the single source of truth for the entire dataset. It is kept locally (gitignored) and must be present for `dev` / `build` / `deploy`. Vite serves it at the absolute path `/revenue-data.csv`.
...
- Deploy with `npm run deploy` (`vite build` + `wrangler deploy`) from a machine that has the local CSV at `public/revenue-data.csv`.
- The CSV is gitignored and must not be committed. Do not connect Cloudflare Pages to GitHub auto-builds for this repository — remote builds would lack the dataset.
- The 16.8 MB CSV is under the 25 MiB per-file limit; no R2 or external storage is required.
